...
DASHSCOPE_API_KEY=your_api_key_here DASHSCOPE_BASE_URL=https://dashscope.aliyuncs.com/compatible-mode/v1
...
import os
products_dir = 'products_file' file_handler = FileHandler(base_path=products_dir) scraper = JdProductScraper(file_handler=file_handler)
products = scraper.search_jd_products('洗碗机', page=1) print(f'获取到 {len(products)} 个商品')
...
批量获取图片
results = scraper.batch_get_product_images( skuids[:5], # 前5个商品
csv_filename='all_product_images.csv', delay=5 # 每个商品间隔5秒 )
...
wait_time = 5 # 增加到 8-10 秒可避免 403 错误
time.sleep(wait_time)

### 登录超时设置

python workflow = ProductAnalysisWorkflow( login_timeout=300, # 登录等待时间（秒） model_name="qwen3.5-plus" )
...
python
workflow = ProductAnalysisWorkflow( model_name="qwen-turbo" # 更快的模型 )
...
csv 标题,原价,最终价,历史价,销售店铺,店铺 ID,sku
